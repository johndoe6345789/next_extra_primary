...
void BadgeService::checkAndAwardBadges(
    ...
    *dbClient << sql << userId >>
        [userId, onSuccess,
         onError](const Result& s) {
            ...
            auto pts = s[0]["total_points"]
                .as<std::int64_t>();
            auto sk = s[0]["current_streak"]
                .as<std::int32_t>();
            auto mg = s[0]["msg_count"]
                .as<std::int64_t>();
            ...
            auto add = [&](bool ok,
                ...
                std::string_view nm,
                std::string_view ds) {
                if (ok) cands.push_back(
                    {{"slug", sl}, {"name", nm},
                     {"description", ds}});
            };
            add(pts>=100,  "first_100_points",
                "Century",  "Earned 100 points");
            add(pts>=1000, "first_1000_points",
                "Millennial","Earned 1000 points");
            add(sk>=7,  "streak_7",
            ...
            for (const auto& b : cands)
                ins.insertOne(userId, b,
                    rem, res, onSuccess);
        } >>
        [onError](const DrogonDbException& e) {
            spdlog::error(
                "checkAndAwardBadges error: {}",
                e.base().what());
