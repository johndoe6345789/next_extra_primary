...
auto AuthService::isValidUsername(
...
auto AuthService::isStrongPassword(
...
auto AuthService::generateRandomToken(
    ...
    ErrCallback onError)
{
    ...
    auto display = displayName.empty()
                       ? username
                       : displayName;

    *dbClient << sql
        << email << username << hashed
        << display << confirmToken
        >> [onSuccess, email, username, confirmToken](
               const Result &result) {
            if (result.empty()) {
