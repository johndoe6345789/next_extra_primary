...
void MigrationRollback::rollbackLast(
    ...
    ErrCallback onError)
{
    ...
    *dbClient << sql >>
        [migrationsDir, onSuccess, onError](const Result& result) {
            if (result.empty()) {
            ...
            if (downSql.empty()) {
                onError(k400BadRequest,
                        fmt::format("{} has no DOWN section", fn));
                return;
            }
            auto trans =
                MigrationStateStore::db()->newTransaction();
            const std::string del =
                "DELETE FROM schema_migrations "
                "WHERE filename = $1";
            *trans << downSql >>
                [trans, fn, del, onSuccess,
                 onError](const Result&) {
                    *trans << del << fn >>
                        [fn, onSuccess](const Result&) {
                ...
                [fn, onError](const DrogonDbException& e) {
                    spdlog::error("Rollback {} failed: {}",
                                  fn, e.base().what());
                    onError(k500InternalServerError,
                            fmt::format("Rollback {} failed: {}",
                                        fn, e.base().what()));
                };
        } >>
        [onError](const DrogonDbException& e) {
            spdlog::error("rollbackLast query error: {}",
                          e.base().what());
            onError(k500InternalServerError,
