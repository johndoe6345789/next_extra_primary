    ...
    ErrCallback onError)
{
    ...
    *applyNext = [migrationsDir, pending, idx, done,
                  onSuccess, onError, applyNext]() {
        if (*idx >= pending.size()) {
            onSuccess(*done);
            return;
        }
        ...
        *trans << upSql >>
            [trans, fn, idx, done, onSuccess, onError,
             applyNext](const Result&) {
                *trans << ins << fn >>
                    [fn, idx, done, onSuccess, onError,
                     applyNext](const Result&) {
                    ...
                    [fn, onError](const DrogonDbException& e) {
                        spdlog::error("Record {} error: {}",
                                      fn, e.base().what());
                        onError(k500InternalServerError,
                                fmt::format("Failed to record {}",
                                            fn));
                    };
            } >>
            [fn, onError](const DrogonDbException& e) {
