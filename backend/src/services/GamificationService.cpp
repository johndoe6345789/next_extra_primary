...
GamificationService::GamificationService()
{
    ...
    try {
        std::ifstream ifs("src/constants/gamification.json");
        if (!ifs.is_open()) {
        ...
        spdlog::error("Failed to parse gamification.json: {}", e.what());
        config_ = json::object();
    }
    ...
    std::ranges::sort(
        levels_, [](auto& a, auto& b) { return a.minPoints > b.minPoints; });

    // Streak milestones.
    if (config_.contains("streaks") &&
        config_["streaks"].contains("milestones")) {
        for (auto& m : config_["streaks"]["milestones"]) {
            milestones_.push_back(m.get<std::int32_t>());
        }
    }
...
auto GamificationService::getLevelForPoints(std::int64_t points) const
    -> std::int32_t
{
...
auto GamificationService::getLevelTitle(std::int32_t level) const -> std::string
{
    for (const auto& lv : levels_) {
        if (lv.level == level) {
            return lv.title;
...
auto GamificationService::pointsToNextLevel(std::int64_t currentPoints) const
    -> std::int64_t
{
...
void GamificationService::awardPoints(const std::string& userId,
                                      std::int64_t amount,
...
void GamificationService::checkAndAwardBadges(const std::string& userId,
                                              Callback onSuccess,
                                              ErrCallback onError)
{
    auto dbClient = db();
    ...
    *dbClient << statsSql << userId >> [this, userId, onSuccess,
                                        onError](const Result& stats) {
        if (stats.empty()) {
            onSuccess(json::array());
            return;
        }
        auto points = stats[0]["total_points"].as<std::int64_t>();
        auto streak = stats[0]["current_streak"].as<std::int32_t>();
        auto msgCount = stats[0]["msg_count"].as<std::int64_t>();

        // Build candidate badge list.
        json candidates = json::array();
        if (points >= 100) {
            candidates.push_back({{"slug", "first_100_points"},
                                  {"name", "Century"},
                                  {"description", "Earned 100 points"}});
        }
        if (points >= 1000) {
            candidates.push_back({{"slug", "first_1000_points"},
                                  {"name", "Millennial"},
                                  {"description", "Earned 1000 points"}});
        }
        if (streak >= 7) {
            candidates.push_back({{"slug", "streak_7"},
                                  {"name", "Week Warrior"},
                                  {"description", "7-day streak"}});
        }
        if (streak >= 30) {
            candidates.push_back({{"slug", "streak_30"},
                                  {"name", "Monthly Master"},
                                  {"description", "30-day streak"}});
        }
        if (msgCount >= 10) {
            candidates.push_back({{"slug", "chatty_10"},
                                  {"name", "Conversationalist"},
                                  {"description", "Sent 10 chat messages"}});
        }
        if (msgCount >= 100) {
            candidates.push_back({{"slug", "chatty_100"},
                                  {"name", "Chatterbox"},
        ...
        for (auto& badge : candidates) {
            ...
            *dbInner << ins << slug << name << desc >> [dbInner, userId, badge,
                                                        remaining, result,
                                                        onSuccess](
                                                           const Result& br) {
                if (br.empty()) {
                ...
                auto badgeId = br[0]["id"].as<std::string>();
                ...
                *dbInner << link << userId << badgeId >>
                    [badge, remaining, result, onSuccess](const Result& lr) {
                        if (!lr.empty()) {
                    ...
                    [remaining, result, onSuccess](const DrogonDbException&) {
                        if (--(*remaining) == 0) {
                            onSuccess(*result);
                        }
                    };
        ...
        spdlog::error("checkAndAwardBadges DB error: {}", e.base().what());
        onError(k500InternalServerError, "Internal server error");
    };
}
...
void GamificationService::updateStreak(const std::string& userId,
                                       Callback onSuccess, ErrCallback onError)
{
    auto dbClient = db();
    ...
    *dbClient << sql << userId >> [this, onSuccess](const Result& result) {
        if (result.empty()) {
        ...
        auto current = result[0]["current_streak"].as<std::int32_t>();
        auto longest = result[0]["longest_streak"].as<std::int32_t>();
        ...
        onSuccess({{"currentStreak", current},
...
void GamificationService::getLeaderboard(const std::string& period,
...
void GamificationService::getUserProgress(const std::string& userId,
                                          Callback onSuccess,
                                          ErrCallback onError)
{
    auto dbClient = db();
    ...
    *dbClient << sql << userId >> [this, onSuccess,
                                   onError](const Result& result) {
        if (result.empty()) {
            onError(k404NotFound, "User not found");
            return;
        }
        auto pts = result[0]["total_points"].as<std::int64_t>();
        auto level = getLevelForPoints(pts);
        auto nextLevelPts = pointsToNextLevel(pts);

        json badges;
        try {
            badges = json::parse(result[0]["badges"].as<std::string>());
        } catch (...) {
            badges = json::array();
        }
        ...
        onSuccess(
            {{"level", level},
             ...
             {"points", pts},
             {"nextLevelAt", pts + nextLevelPts},
             {"pointsToNextLevel", nextLevelPts},
             {"currentStreak", result[0]["current_streak"].as<std::int32_t>()},
             {"longestStreak", result[0]["longest_streak"].as<std::int32_t>()},
             {"badges", badges}});
    } >> [onError](const DrogonDbException& e) {
