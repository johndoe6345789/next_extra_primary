...
auto UserService::rowToJson(const Row &row) -> json
{
    json user = {
        ...
        {"emailConfirmed",
         row["email_confirmed"].as<bool>()},
        {"totalPoints",
         row["total_points"].as<std::int64_t>()},
        {"currentStreak",
...
void UserService::getUserByEmail(
    ...
    ErrCallback onError)
{
    ...
    for (const auto &col : kEditableFields) {
        if (fields.contains(col)
            && fields[col].is_string()) {
            setClauses.push_back(
                fmt::format("{} = ${}",
                            col, ++paramIdx));
            values.push_back(
                fields[col].get<std::string>());
        }
    }
...
void UserService::getUserBadges(
