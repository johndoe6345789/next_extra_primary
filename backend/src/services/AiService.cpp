...
{
    if (message.empty()) {
        onError(drogon::k400BadRequest, "Message must not be empty");
        return;
    }
    ...
    historyStore_.storeMessage(userId, "user", message, provStr, "");
        ...
        [this, userId, message, provider, provStr, onSuccess,
         onError](json history) {
            ...
            auto handleReply = [this, userId, provStr, onSuccess](json reply) {
                ...
                historyStore_.storeMessage(userId, "assistant", content,
                                           provStr, model);
            ...
            if (provider == AiProvider::OPENAI) {
                openAiClient_.call(history, handleReply, onError);
            } else {
                claudeClient_.call(history, handleReply, onError);
            }
        });
}
