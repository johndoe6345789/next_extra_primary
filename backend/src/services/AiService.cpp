...
AiService::AiService()
{
    ...
    if (claudeApiKey_.empty()) {
        spdlog::warn(
            "ANTHROPIC_API_KEY not set; Claude "
            "calls will fail");
    }
    ...
    ErrCallback onError)
{
    ...
    loadHistory(
        ...
        [this, userId, message, provider, provStr,
         onSuccess, onError](json history) {
            ...
            if (provider == AiProvider::OPENAI) {
                callOpenAi(history, handleReply,
                           onError);
            } else {
                callClaude(history, handleReply,
                           onError);
            }
        });
}
    ...
    ErrCallback onError)
{
    ...
    req->addHeader("anthropic-version",
                   "2023-06-01");
    ...
    ErrCallback onError)
{
    ...
    req->addHeader(
        "Authorization",
        fmt::format("Bearer {}", openaiApiKey_));
    req->setContentTypeCode(CT_APPLICATION_JSON);
    ...
    const std::string &model)
{
    ...
    *dbClient << sql
        ...
        >> [](const DrogonDbException &e) {
            spdlog::error(
                "storeMessage DB error: {}",
                e.base().what());
        };
}

void AiService::loadHistory(
    const std::string &userId,
    std::size_t limit,
