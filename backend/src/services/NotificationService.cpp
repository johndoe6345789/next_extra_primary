    ...
    Callback onSuccess, ErrCallback onError)
{
    ...
    *dbClient << sql << userId << title << body << type << metadata.dump() >>
        [onSuccess](const Result& result) {
            ...
            json notif = {{"id", r["id"].as<std::string>()},
...
void NotificationService::getNotifications(const std::string& userId,
                                           std::int32_t page,
                                           std::int32_t perPage,
                                           Callback onSuccess,
                                           ErrCallback onError)
{
    ...
    *dbClient << countSql << userId >> [dbClient, userId, page, perPage, offset,
        ...
        *dbClient << dataSql << userId << perPage << offset >>
        ...
        spdlog::error("getNotifications count "
                      "DB error: {}",
                      e.base().what());
        onError(k500InternalServerError, "Internal server error");
    };
...
void NotificationService::getUnreadCount(const std::string& userId,
                                         Callback onSuccess,
                                         ErrCallback onError)
{
    auto dbClient = db();
    ...
    *dbClient << sql << userId >> [onSuccess](const Result& result) {
        ...
        spdlog::error("getUnreadCount DB error: {}", e.base().what());
        onError(k500InternalServerError, "Internal server error");
    };
}
...
void NotificationService::deleteNotification(const std::string& notificationId,
                                             const std::string& userId,
                                             Callback onSuccess,
                                             ErrCallback onError)
{
...
void NotificationService::notifyBadgeEarned(const std::string& userId,
                                            const json& badge)
{
    auto name = badge.value("name", "Unknown Badge");
    createNotification(
        userId, fmt::format("Badge Earned: {}", name),
        ...
        [](drogon::HttpStatusCode, const std::string& err) {
            spdlog::error("notifyBadgeEarned failed: {}", err);
        });
}

void NotificationService::notifyLevelUp(const std::string& userId,
                                        std::int32_t newLevel)
{
    createNotification(
        userId, fmt::format("Level Up! You're now level {}", newLevel),
        fmt::format("You've reached level {}. Keep going!", newLevel),
        "level_up", {{"level", newLevel}}, [](const json&) {},
        [](drogon::HttpStatusCode, const std::string& err) {
            spdlog::error("notifyLevelUp failed: {}", err);
        });
}

void NotificationService::notifyStreakMilestone(const std::string& userId,
                                                std::int32_t days)
{
    createNotification(
        userId, fmt::format("{}-Day Streak!", days),
        fmt::format("Amazing! You've maintained a {}-day "
                    "streak.",
                    days),
        "streak", {{"days", days}}, [](const json&) {},
        [](drogon::HttpStatusCode, const std::string& err) {
            spdlog::error("notifyStreakMilestone failed: {}", err);
        });
}
