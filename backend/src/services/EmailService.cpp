...
EmailService::EmailService()
{
    ...
    smtpPort_ = static_cast<std::uint16_t>(
        std::stoi(env("SMTP_PORT", "587")));
    smtpUser_ = env("SMTP_USER", "");
    smtpPass_ = env("SMTP_PASS", "");
    smtpFrom_ = env("SMTP_FROM",
                     "noreply@nextra.app");
    baseUrl_  = env("APP_BASE_URL",
                     "http://localhost:3000");

    loadTemplates();

    spdlog::info(
        "EmailService configured: host={}, port={}",
        smtpHost_, smtpPort_);
}
...
void EmailService::loadTemplates()
{
    try {
        std::ifstream ifs(
            "src/constants/email_templates.json");
        if (!ifs.is_open()) {
        ...
        if (ifs.is_open()) {
            ...
            spdlog::warn(
                "email_templates.json not found; "
    ...
    const json &vars) const -> std::string
{
    ...
    for (auto &[key, value] : vars.items()) {
        ...
        while ((pos = result.find(
               ...
               != std::string::npos) {
            result.replace(
                pos, placeholder.size(), replacement);
    ...
    const std::string &confirmToken)
{
    ...
    // blocked by SMTP I/O.
    std::thread([this, email, username,
                 confirmToken]() {
        try {
            auto tmpl = templates_.value(
                "welcome", json::object());
            auto subjectTmpl = tmpl.value(
                "subject", "Welcome to Nextra!");
