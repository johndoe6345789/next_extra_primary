...
auto MigrationService::discoverFiles() const
    -> std::vector<std::string>
{
    std::vector<std::string> files;
    if (!fs::exists(migrationsDir_)
        || !fs::is_directory(migrationsDir_)) {
        spdlog::warn(
    ...
    for (const auto &entry :
         fs::directory_iterator(migrationsDir_)) {
...
auto MigrationService::readFile(
    const std::string &path) -> std::string
{
    std::ifstream ifs(path);
    if (!ifs.is_open()) {
        throw std::runtime_error(
            fmt::format("Cannot open file: {}",
                        path));
    }
...
auto MigrationService::extractUp(
...
auto MigrationService::extractDown(
    const std::string &sql) -> std::string
{
    auto pos = sql.find("-- DOWN");
    if (pos == std::string::npos) {
...
void MigrationService::ensureTable(
