...
void AiClaudeClient::call(const json& messages, Callback onSuccess,
                          ErrCallback onError)
{
    if (apiKey_.empty()) {
    ...
    json body = {
        {"model", kModel}, {"max_tokens", kMaxTokens}, {"messages", messages}};
        ...
        try {
            ...
            if (resp->statusCode() != k200OK) {
                auto msg = j.value("error", json::object())
                               .value("message", "Unknown Claude error");
            ...
            spdlog::error("Claude parse error: {}", e.what());
            onError(k502BadGateway, "Invalid Claude API response");
        }
    });
}
} // namespace services
