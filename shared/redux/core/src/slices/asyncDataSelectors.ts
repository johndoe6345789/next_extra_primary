...
/** Selector: Get data from specific request */
export const selectAsyncData = (
  state: { asyncData: AsyncDataState },
  id: string
) => state.asyncData.requests[id]?.data;

/** Selector: Get error from specific request */
export const selectAsyncError = (
  state: { asyncData: AsyncDataState },
  id: string
) => state.asyncData.requests[id]?.error;

/** Selector: Check if request is loading */
export const selectAsyncLoading = (
  state: { asyncData: AsyncDataState },
  id: string
) =>
  state.asyncData.requests[id]?.status ===
  'pending';

/** Selector: Check if request is refetching */
export const selectAsyncRefetching = (
  state: { asyncData: AsyncDataState },
  id: string
) =>
  state.asyncData.requests[id]?.isRefetching ??
  false;
