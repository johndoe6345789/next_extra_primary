...
static void register_run(CLI::App& app) {
    ...
    sub->callback([&port, &config]() {
        ...
        std::string cmd =
            fmt::format("./build/Release/nextra-api");
        if (port != 8080) {
...
int main(int argc, char** argv) {
    CLI::App app{
        "manager: nextra-api project management tool"
    };
    ...
    std::string filter;
    test->add_option(
        "-f,--filter", filter, "Test name filter"
    );
    ...
    // ---- docker ----
    auto* docker = app.add_subcommand(
        "docker", "Docker compose operations"
    );
    std::string docker_action;
    docker->add_option(
        "action", docker_action,
        "Action: build, up, down, logs"
    )->required();
    docker->callback([&docker_action]() {
        int rc = manager::DockerCmd::execute(
            docker_action
        );
        if (rc != 0) {
            fmt::print(
                "[manager] Docker failed ({})\n", rc
            );
        }
    });

    // ---- lint ----
    auto* lint = app.add_subcommand(
        "lint", "Run clang-format and clang-tidy checks"
    );
    lint->callback([]() {
        int rc = manager::LintCmd::lint();
        if (rc != 0) {
            fmt::print(
                "[manager] Lint found issues ({})\n", rc
            );
        }
    });

    // ---- fmt ----
    auto* fmt_cmd = app.add_subcommand(
    ...
    auto* gen = app.add_subcommand(
        "generate", "Generate cmake or models"
    );
    std::string gen_target;
    gen->add_option(
        "target", gen_target,
        "What to generate: cmake, models"
    )->required();
    gen->callback([&gen_target]() {
        int rc = manager::GenerateCmd::execute(
            gen_target
        );
        if (rc != 0) {
            fmt::print(
