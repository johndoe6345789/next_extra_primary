...
namespace manager
{
static bool evalWhen(const std::string& c, const WorkflowCtx& ctx)
{
    if (c.empty())
        return true;
    auto x = ctx.expand(c);
...
static int stepCheck(const nlohmann::json& s, WorkflowCtx& ctx)
{
...
static int stepShell(const nlohmann::json& s, WorkflowCtx& ctx)
{
    return shell(s.value("tag", "workflow"),
                 ctx.expand(s["command"].get<std::string>()));
}
static int stepCapture(const nlohmann::json& s, WorkflowCtx& ctx)
{
    ctx.set(s["var"].get<std::string>(),
            capture(ctx.expand(s["command"].get<std::string>())));
    return 0;
}
static int stepSet(const nlohmann::json& s, WorkflowCtx& ctx)
{
...
static int stepPrint(const nlohmann::json& s, WorkflowCtx& ctx)
{
    fmt::print("{}\n", ctx.expand(s["text"].get<std::string>()));
    return 0;
}
static int stepForeach(const nlohmann::json& s, WorkflowCtx& ctx)
{
    auto out = capture(ctx.expand(s["command"].get<std::string>()));
    auto var = s["var"].get<std::string>();
...
{
    if (s.contains("when") && !evalWhen(s["when"].get<std::string>(), ctx))
        return 0;
    auto t = s["type"].get<std::string>();
    if (t == "check")
        return stepCheck(s, ctx);
    if (t == "shell")
        return stepShell(s, ctx);
    if (t == "capture")
        return stepCapture(s, ctx);
    if (t == "set")
        return stepSet(s, ctx);
    if (t == "print")
        return stepPrint(s, ctx);
    if (t == "foreach")
        return stepForeach(s, ctx);
    if (t == "write")
        return stepWrite(s, ctx);
    if (t == "retry")
        return stepRetry(s, ctx);
    if (t == "template")
        return stepTemplate(s, ctx);
