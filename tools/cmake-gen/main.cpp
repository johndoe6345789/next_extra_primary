 ...
 */
static std::vector<std::string> discover_sources(
    ...
    for (const auto& entry :
        ...
        if (!exclude_patterns.empty()
            && matches_any(fname, exclude_patterns)) {
            continue;
        }
...
int main(int argc, char** argv) {
    CLI::App app{
        "cmake-gen: Generate CMakeLists.txt from "
    ...
    try {
        ...
        for (const auto& entry :
             fs::directory_iterator(tmpl_abs)) {
            if (!entry.is_regular_file()) {
                continue;
            }
            auto fname = entry.path().filename().string();
            if (fname.size() < 10
                || fname.substr(fname.size() - 9)
                       != ".cmake.j2") {
                continue;
            }
            ...
            std::string tmpl_content(
                (std::istreambuf_iterator<char>(tfs)),
                std::istreambuf_iterator<char>()
            );

            std::string rendered =
                env.render(tmpl_content, ctx);
