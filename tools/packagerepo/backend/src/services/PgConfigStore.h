...
public:
    ...
    static int defaultRepoType()
    {
        auto r = DbPool::get()->execSqlSync(
            "SELECT id FROM repo_types "
            "WHERE name='generic'");
        return r.empty() ? 0 : r[0]["id"].as<int>();
    }

    /// @brief Get entity definition for a repo type.
    static Json::Value entityDef(int repoType,
                                 const std::string& name)
    {
        auto r = DbPool::get()->execSqlSync(
            "SELECT fields,constraints,primary_key "
        ...
        Json::Value out;
        out["fields"] = parseJson(
            r[0]["fields"].as<std::string>());
        out["constraints"] = parseJson(
            r[0]["constraints"].as<std::string>());
        out["primary_key"] = parseJson(
            r[0]["primary_key"].as<std::string>());
        return out;
    }
    ...
    {
        auto r = DbPool::get()->execSqlSync(
            "SELECT key,value FROM features");
        Json::Value out(Json::objectValue);
        for (const auto& row : r) {
            out[row["key"].as<std::string>()] = parseJson(
    ...
    {
        auto r = DbPool::get()->execSqlSync(
            "SELECT name,actions FROM auth_scopes");
        Json::Value out(Json::arrayValue);
        for (const auto& row : r) {
            Json::Value s;
            s["name"] = row["name"].as<std::string>();
            s["actions"] = parseJson(
                row["actions"].as<std::string>());
            out.append(s);
        }
        return out;
    }
