...
class BucketStore
{
  public:
    ...
    static bool create(const std::string& name, const std::string& region)
    {
        try {
            DbPool::get()->execSqlSync("INSERT INTO buckets (name, region) "
                                       "VALUES ($1, $2)",
                                       name, region);
            return true;
        } catch (...) {
    ...
    {
        auto r =
            DbPool::get()->execSqlSync("SELECT * FROM buckets ORDER BY name");
    ...
    {
        auto r = DbPool::get()->execSqlSync("DELETE FROM buckets WHERE name=$1 "
                                            "RETURNING id",
                                            name);
        return !r.empty();
    }
